...
def print_receipt(file_name, products_dict):
    ...
    with open(file_name, "rt") as csv_file:
        ...
        for row_list in reader:
            ...
            if len(row_list) != 0:
                ...
                subtotal += item_total
                # add the product name, requested quantity and price to the receipt dictionary
                # eliminating duplicate product lines
                if name in receipt_dict:
                    receipt_dict[name][RD_QUANTITY_INDEX] += quantity
                else:
                    receipt_dict[name] = [name, quantity, price]
                # add name to customer product list
                customer_product_list.append(name)

    # print receipt products from dictionary using for loop
    for key, product in receipt_dict.items():
    ...
    print_coupon(customer_product_list, 0.15)


def print_coupon(customer_product_list, discount):
    """
    Select a random product from customer product list and create a coupon.
    Parameters
      customer_product_dict: the product dictionary
      discount: the discount to be offered in decinmal
    Return: None
    """
