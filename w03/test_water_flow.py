...
def test_convert_kPa_to_psi():
    """Verify that the convert_kPa_to_psi function returns correct results.

    Parameters: none
    Return: nothing
    """
    # Use approx with absolute tolerance
    assert convert_kPa_to_psi(158.7) == approx(23.017, abs=0.001)
    assert convert_kPa_to_psi(0) == approx(0.000, abs=0.001)
    assert convert_kPa_to_psi(-100) == approx(-14.504, abs=0.001)
